import { Archive, Bot, CreditCard, FileSearch, FileText, ImageIcon, Mic, Pencil, Play, Plus, RotateCcw, Save, ShieldAlert, Wand2, XCircle } from "lucide-react";
...
export function SettingsPage({
    ...
    { key: "uploaded", label: "Uploaded", render: (file) => formatDate(file.created_at) },
    {
      ...
      render: (file) => (
        <div className="flex justify-end gap-2">
          ...
          <Button
            aria-label={`Analyze document ${file.original_file_name}`}
            className="size-9 px-0"
            disabled={!canManage}
            icon={<FileSearch className="size-4" aria-hidden="true" />}
            title={`Analyze document ${file.original_file_name}`}
            onClick={() => openFileModal(file, "analysis")}
          >
          ...
          {isSupportedAudioAttachment(file) ? (
            <Button
              aria-label={`Transcribe audio ${file.original_file_name}`}
              className="size-9 px-0"
              disabled={!canManage}
              icon={<Mic className="size-4" aria-hidden="true" />}
              title={`Transcribe audio ${file.original_file_name}`}
              onClick={() => openFileModal(file, "audio")}
            >
  ...
  function openFileModal(file: Attachment, mode: "summary" | "analysis" | "image" | "audio" = "summary"): void {
    setEditingFile(file);
    setFormError(null);
    ...
    void loadFileAIInsight(file, mode);
  }
  ...
  async function loadFileAIInsight(file: Attachment, mode: "summary" | "analysis" | "image" | "audio" = fileAIInsightMode): Promise<void> {
    ...
    try {
      const job =
        mode === "audio"
          ? await api.latestFileAITranscription(file.id, selectedCompany.id)
          : mode === "image"
      ...
      setFileAISummaryError(
        caughtError instanceof Error
          ? caughtError.message
          : mode === "audio"
            ? "Unable to load the latest audio transcription."
            : mode === "image"
    ...
    try {
      const job =
        fileAIInsightMode === "audio"
          ? await api.generateFileAITranscription(editingFile.id, selectedCompany.id)
  ...
  const audioTranscriptionRealSupported = Boolean(aiProviderStatus?.supported_real_job_types.includes("audio_transcription_safe"));
  const audioTranscriptionSupportLabel = displayedProviderMode === "mock" ? "Mock only" : audioTranscriptionRealSupported ? "Supported" : "Not supported";
  const audioTranscriptionSupportTone: "blue" | "green" | "amber" = displayedProviderMode === "mock" ? "blue" : audioTranscriptionRealSupported ? "green" : "amber";
          ...
          <div className="grid gap-3 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="febgrid-muted-surface rounded-lg p-4">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                ...
                <div className="rounded-lg border border-grid-200 bg-white/70 p-3 sm:col-span-2">
                  <p className="text-xs font-black uppercase text-ink-500">Audio transcription</p>
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    <Badge label={audioTranscriptionSupportLabel} tone={audioTranscriptionSupportTone} />
                    <span className="text-xs font-semibold text-ink-500">
                      {audioTranscriptionRealSupported
                        ? "Current real provider declares audio transcription support."
                        : "Current provider/model does not support real audio transcription yet; mock mode remains available."}
                    </span>
                  </div>
                </div>
              </div>
